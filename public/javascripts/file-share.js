...
fileShare.on('receive_chunk', function (data) {
    ...
    console.log('receive chunk (file share) ' + meta.storage);
...
fileShare.on('new_sharer', function (data) {
    ...
    meta.queue.push(someone);
    fileShareSendChunk(meta.rank, meta.storage.length, meta,hashCode,meta.queue.length-2);
...
function fileShareSendChunk(i,end,meta,hashCode,cnt)
{
    ...
    if(i<end)
    {
        ...
        i+=cnt;
        setTimeout(function () {
            fileShareSendChunk(i, end, meta,hashCode,cnt);
        }, 500);
    }
    
}

function holderSendChunk(hashCode,start){
	...
	for(var i in receivers){
		if(start>=end) break;
		fileShare.send(receivers[i],'fileShare',{
			'event':'receive_chunk',
			'data':{
				'chunk': meta.storage[start],
				'hashCode': meta.hashCode,
				'index':index,
	...
	if(index<end)
		setTimeout(function(){
			holderSendChunk(meta.hashCode,index);
		}, 500)
}
